        ...
        for (var i = 0; i < message.member._roles.length; i++) {
            if (!message.member.roles.cache.some(role => role.name === roles[i])) {
               ...
               node.login('https://panel.darkst.one/', config.APIKey, (logged_in, msg) => {
                ...
                return node.restartServer(serverID)
                    .then((res) => {
                        message.channel.send(`${args1.toUpperCase()} has restarted successfully.`);
                    })
                    .catch(err => {
                        message.channel.send("There was an error attempting to restarting the server. (Try \`d!kill\`)");
                        console.log(err)
                    })
            }
        }
